...
###################################################################################################
    ...
    # Using a loop here but this can/will be static
        for i in range (0, num_step_wus[step]):
            ...
            dare_config.set(section_name, "arguments", "-p %s %s"%(working_dirs[r],cactus_working_dirs[r]))
...
###################################################################################################    
    ...
    # Using a loop here but this can/will be static
        for i in range (0, num_step_wus[step]):   
            ...
            dare_config.set(section_name, "source_url",ft_list[i] )
            dare_config.set(section_name, "dest_url", resource_conf["filetransfer_url"] + ':' + cactus_working_dirs[r])
...
###################################################################################################
    ...
    step_conf_file = os.path.join(DARE_HOME, 'examples/cactus/cactus_1_resource.cfg')
    ...
    for i in range (0, num_step_wus[step]):   
        ...
        dare_config.set(section_name, "arguments", step_conf["common_arguments"])
        dare_config.set(section_name, "environment",step_resource_conf["environment"]) 
        dare_config.set(section_name, "working_directory", cactus_working_directory)
...
###################################################################################################
    ...
    #add tasks for each step
    step_conf_file = os.path.join(DARE_HOME, 'examples/cactus/cactus_2_resource.cfg')
    step_config = ConfigParser.ConfigParser()
    step_config.read(step_conf_file) 
    step_conf = dict_section(step_config, "common")
    step_resource_conf = dict_section( step_config, resources_used[0])
    ...
    for i in range (0, num_step_wus[step]):   
        ...
        dare_config.set(section_name, "number_of_processes", step_conf["number_of_processes"])
        dare_config.set(section_name, "spmd_variation", step_conf["spmd_variation"])
        # change to arguments of thorn file name
        dare_config.set(section_name, "arguments", step_conf["common_arguments"] + ' ' + os.path.join(cactus_working_directory, 'einsteintoolkit.th' ))
        dare_config.set(section_name, "environment",step_resource_conf["environment"]) 
        dare_config.set(section_name, "working_directory", cactus_working_directory)
        dare_config.set(section_name, "output", os.path.join(working_directory, "stdout-"+ str(dare_uuid) +"-"+str(wus_count)+".txt"))
        dare_config.set(section_name, "error", os.path.join(working_directory, "stderr-"+ str(dare_uuid) +"-"+str(wus_count)+".txt" ))
...
###################################################################################################    
    ...
    for r in range (0,len(resources_used)):    
        ...
        dare_config.set(section_name, "appname", "file_transfer")
        dare_config.set(section_name, "fs_type", "scp")
        dare_config.set(section_name, "source_url", to_this_file )
        dare_config.set(section_name, "dest_url", resource_conf["filetransfer_url"] + ':' + os.path.join(cactus_working_dirs[r],'Cactus/simfactory/etc/defs.local.ini') )
    ...
    for r in range (0,len(resources_used)):    
        ...
        dare_config.set(section_name, "appname", "file_transfer")
        dare_config.set(section_name, "fs_type", "scp")
        dare_config.set(section_name, "source_url", os.path.join(PWD, 'cactus','cactus_installer.py'  ))
        dare_config.set(section_name, "dest_url", resource_conf["filetransfer_url"] + ':' + os.path.join(cactus_working_dirs[r],'cactus_installer.py') )
...
####################################################################################################
    ...
    step_conf_file = os.path.join(DARE_HOME, 'examples/cactus/cactus_3_resource.cfg')
    step_config = ConfigParser.ConfigParser()
    step_config.read(step_conf_file) 
    step_conf = dict_section( step_config, "common")
    step_resource_conf = dict_section(step_config, resources_used[0])
    ...
    for i in range (0, num_step_wus[step]):   
        ...
        dare_config.set(section_name, "number_of_processes", step_conf["number_of_processes"])
        dare_config.set(section_name, "spmd_variation", step_conf["spmd_variation"])
        dare_config.set(section_name, "arguments", 'python ' + os.path.join(cactus_working_directory, 'cactus_installer.py') )
        dare_config.set(section_name, "environment",step_resource_conf["environment"]) 
        dare_config.set(section_name, "working_directory", cactus_working_directory + '/Cactus/')
        dare_config.set(section_name, "output", os.path.join(working_directory, "stdout-"+ str(dare_uuid) +"-"+str(wus_count)+".txt"))
        dare_config.set(section_name, "error", os.path.join(working_directory, "stderr-"+ str(dare_uuid) +"-"+str(wus_count)+".txt" ))
        dare_config.set(section_name, "appname", "simple")
        dare_config.set(section_name, "resource" , 0)
...
####################################################################################################
    ...
    step_conf_file = os.path.join(DARE_HOME, 'examples/cactus/cactus_4_resource.cfg')
    step_config = ConfigParser.ConfigParser()
    step_config.read(step_conf_file) 
    step_conf = dict_section( step_config, "common")
    step_resource_conf = dict_section(step_config, resources_used[0])
    ...
    for i in range (0, num_step_wus[step]):   
        ...
        dare_config.set(section_name, "number_of_processes", step_conf["number_of_processes"])
        dare_config.set(section_name, "spmd_variation", step_conf["spmd_variation"])
# change parameter file here
        dare_config.set(section_name, "arguments", "--parfile="+ os.path.join(cactus_working_directory, job_conf['paramname'])\
                                                   + ' ' + step_conf["common_arguments"]
                                                   + ' ' + step_resource_conf["arguments"]\
                                                   + ' ' + cactus_sim_name ) 
        dare_config.set(section_name, "environment",step_resource_conf["environment"]) 
        dare_config.set(section_name, "working_directory", os.path.join(cactus_working_directory,'Cactus'))
        dare_config.set(section_name, "output", os.path.join(working_directory, "stdout-"+ str(dare_uuid) +"-"+str(wus_count)+".txt"))
        dare_config.set(section_name, "error", os.path.join(working_directory, "stderr-"+ str(dare_uuid) +"-"+str(wus_count)+".txt" ))
        dare_config.set(section_name, "appname", "simple")
        dare_config.set(section_name, "resource" , 0)
