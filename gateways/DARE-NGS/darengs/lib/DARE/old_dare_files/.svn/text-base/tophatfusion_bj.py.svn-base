...
def sub_jobs_submit( jd_executable, job_type, affinity ,  subjobs_start,  number_of_jobs, jd_number_of_processes):
        ...
        for i in range(subjobs_start, int(number_of_jobs) + int(subjobs_start) ):
            ...
            if job_type == "tophat1":
                jd.arguments = ["-o", "/home/cctsg/bio_data/tophatfusion/tophat_MCF7",# "%s" %(bfast_reads_num[affinity]),  
                                "-p", "2",
                                "--allow-indels",
                                "--no-coverage-search",
                                "-r", "0",
                                ...
                                "/opt/bowtie-0.12.7/indexes/hg19/hg19",  
                                "/home/cctsg/bio_data/tophatfusion/SRR064286_1.fastq",
                                "/home/cctsg/bio_data/tophatfusion/SRR064286_2.fastq",
                               ]
               
             elif job_type == "tophat1":  
               jd.arguments = [   
                                "-p", "2", 
                                "/opt/bowtie-0.12.7/indexes/hg19/hg19",
                                "--num-fusion-reads", "1" ,
            ...
            jd.working_directory = "/home/cctsg/bio_data/tophatfusion/"
            ...
            jd.environment = ["PATH=${PATH}:/opt/tophatfusion-0.1.0/bin/:/opt/bowtie-0.12.7/"]
...
if __name__ == "__main__":
    ...
    try:  
        ...
        for i in range (0, len(resources_used)):         
            ...
            logger.info( " TOPHAT 1 resource " + str(i))
        ...
        logger.info("TOPHAT  1 Runtime: " + str( matches_runtime) )       
        ...
        for i in range (0, len(resources_used)):         
            sub_jobs_submit("bfast","localalign", i , total_number_of_jobs 
                           , resources_job_count[i],int(bfast_num_cores[i]))
            logger.info( "TOPHAT 2 resource " + str(i))
            logger.info( "total_number_of_jobs " + str(total_number_of_jobs))
            logger.info( "resources_job_count " + str(resources_job_count[i]))
            logger.info( "int(bfast_num_cores" + str(bfast_num_cores[i]))            
            total_number_of_jobs = total_number_of_jobs + int(resources_job_count[i])
        ...
        logger.info("TOPHAT 2  Runtime: " + str( localalign_runtime) )
